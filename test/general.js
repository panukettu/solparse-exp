...
describe('Parses doc_examples_long.sol', function () {
	it('parses test.sol as the antlr', function () {
		try {
			let all = SolidityParser.parseFile('./test/doc_examples_long.sol', true);
			let x = all;
		} catch (error) {
			console.log('./test/doc_examples_long.sol', error.message);
			let x = error;
		}
	});
});
...
describe('Parse comments', () => {
	...
	it('should parse comments', () => {
		...
		const expectedCommLen = 65;

		if (comments.length !== expectedCommLen) {
			throw new Error(`there should be ${expectedCommLen} comment objects, got: ${comments.length}`);
		}
