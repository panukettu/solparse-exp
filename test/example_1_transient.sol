...
contract BatchFacet is IBatchFacet, Modifiers {
    ...
    Kek internal data = Kek({val: 1e18});
    /// @inheritdoc IBatchFacet
    function batchCall(
        bytes[] calldata _calls,
        bytes[] calldata _updateData
    ) external payable usePyth(_updateData) {
...
function decodeErrorData(
    bytes calldata _errorData
) pure returns (uint256 timestamp, bytes[] memory results) {
